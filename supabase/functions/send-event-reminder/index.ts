...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { event_id, email, user_id }: ReminderRequest = await req.json();
      ...
      html: `
        ...
        <head>
          <style>
            ...
            .button { display: inline-block; background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: #0a0a0b; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; margin-top: 15px; }
          </style>
        </head>
        <body>
          <div class="container">
